...
resource "aws_iam_role" "remediate" {
  name = "${var.project_name}-03-remediate-role"
...
# The responder can only revoke ingress on security groups tagged for this project,
# never on arbitrary groups in the account. Least privilege for the remediator too.
resource "aws_iam_role_policy" "remediate_ec2" {
  name = "${var.project_name}-03-remediate-ec2"
...
resource "aws_iam_role_policy" "remediate_persist" {
  name = "${var.project_name}-03-remediate-persist"
  role = aws_iam_role.remediate.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
...
resource "aws_lambda_function" "remediate" {
  function_name    = "${var.project_name}-03-remediate"
